...
################## Tree and metadata 
tree_file <- "/full_path_to/wd/RdRp_scan/analysis/phylo/Monjiviricetes_clade/tree/aa_tree/Monjiviricetes_clade3_aa_aln_outg.fasta.gap50_clean.fasta.contree"
...
tip_mdf_brief[which(tip_mdf_brief$nt_accession == "MZ202303"),]$host_taxon <- "Culicidae"
...
tip_mdf_brief$host_taxon1 <- factor(tip_mdf_brief$host_taxon1, ordered = TRUE,
                                   levels = c('Culex pipiens','Culex quinquefasciatus','Culex tarsalis','Culex tritaeniorhynchus','Culex erythrothorax',    
                                              'Culex',
                                              'Deinocerites',
                                              'Ochlerotatus',
...
tip_mdf_brief[which(tip_mdf_brief$Country %in% 
                      c("Canada", "USA", "Mexico", "Guadeloupe", "Trinidad and Tobago", "Grenada")),
...
virus_clade <- c("MZ771217", "XiangYun mono-chu-like virus 11", "Elisy virus", "Cimo rhabdovirus III", 
                 "Stang virus", "Antlested virus", "Wuhan Mosquito Virus 9", "Atrato Rhabdo-like virus 2",
                 "Grenada mosquito rhabdovirus 1", "Cururu virus")

clades_df <- tibble(virus_clade)
clades_df$node <- NA

clades_df[which(clades_df$virus_clade == "MZ771217"), ]$node <- 60
clades_df[which(clades_df$virus_clade == "XiangYun mono-chu-like virus 11"), ]$node <- 59
clades_df[which(clades_df$virus_clade == "Elisy virus"), ]$node <- 118
clades_df[which(clades_df$virus_clade == "Cimo rhabdovirus III"), ]$node <- 56
clades_df[which(clades_df$virus_clade == "Stang virus"), ]$node <- 117
clades_df[which(clades_df$virus_clade == "Antlested virus"), ]$node <- 111
clades_df[which(clades_df$virus_clade == "Wuhan Mosquito Virus 9"), ]$node <- 113
clades_df[which(clades_df$virus_clade == "Atrato Rhabdo-like virus 2"), ]$node <- 47
clades_df[which(clades_df$virus_clade == "Grenada mosquito rhabdovirus 1"), ]$node <- 63
clades_df[which(clades_df$virus_clade == "Cururu virus"), ]$node <- 42
...
clades_df$tip <- FALSE
clades_df[which(clades_df$virus_clade_essential %in% c("MZ771217", 
                                                       "XiangYun mono-chu-like virus 11",
                                                       "Cimo rhabdovirus III",
                                                       "Atrato Rhabdo-like virus 2",
                                                       "Cururu virus")),]$tip <- TRUE
...
ggt2_hm <- ggt2_hm + scale_fill_manual(breaks=c(# 'Anopheles',
  ...
  # 'Culiseta',
  'Culex pipiens','Culex quinquefasciatus','Culex tarsalis','Culex tritaeniorhynchus','Culex erythrothorax',
  'Culex',
  'Deinocerites',
...
values=c( # "black",
  ...
  "#A89CE6","#9A8CE2","#8D7CDE","#7F6DDA","#715DD6",       
  "#332288",
...
# 
# print(ggt2_hm)
ggt2_hm_file <- "/full_path_to/wd/RdRp_scan/analysis/phylo/Monjiviricetes_clade/Monjiviricetes_clade3_aa_aln.ggtree_hm.pdf"
ggsave(plot = ggt2_hm, filename = ggt2_hm_file, device = "pdf", width = 10, height = 14, units = "cm")

# ggt2_hm_file_legend <- "/full_path_to/wd/RdRp_scan/analysis/phylo/Monjiviricetes_clade/Monjiviricetes_clade3_aa_aln.ggtree_hm_legend.pdf"
...
print(ggt_sup2_hm)
ggt_sup2_hm_file <- "/full_path_to/wd/RdRp_scan/analysis/phylo/Monjiviricetes_clade/Monjiviricetes_clade3_aa_aln.ggtree_hm_supplementary.pdf"
