...
################## Tree and metadata 
tree_file <- "/full_path_to/wd/RdRp_scan/analysis/phylo/Monjiviricetes_clade/tree/aa_tree/Monjiviricetes_clade4_aa_aln_outg.fasta.gap50_clean.fasta.contree"
...
virus_clade <- c("Wuhan Ant Virus", "Atrato Rhabdo-like virus 3", 
                 "Joutseno rhabdovirus 1", "Retriended virus", "Primus virus", 
                 "XiangYun mono-chu-like virus 4", "San Gabriel mononegavirus",
                 "Reloonsia virus", "Preculath virus")

clades_df <- tibble(virus_clade)
clades_df$node <- NA

clades_df[which(clades_df$virus_clade == "Wuhan Ant Virus"), ]$node <- 35
clades_df[which(clades_df$virus_clade == "Atrato Rhabdo-like virus 3"), ]$node <- 33
clades_df[which(clades_df$virus_clade == "Joutseno rhabdovirus 1"), ]$node <- 34
clades_df[which(clades_df$virus_clade == "Retriended virus"), ]$node <- 30
clades_df[which(clades_df$virus_clade == "Primus virus"), ]$node <- 31
clades_df[which(clades_df$virus_clade == "XiangYun mono-chu-like virus 4"), ]$node <- 32
clades_df[which(clades_df$virus_clade == "San Gabriel mononegavirus"), ]$node <- 59
clades_df[which(clades_df$virus_clade == "Reloonsia virus"), ]$node <- 56
clades_df[which(clades_df$virus_clade == "Preculath virus"), ]$node <- 39
...
clades_df$tip <- FALSE
clades_df[which(clades_df$virus_clade_essential %in% c("Wuhan Ant Virus",
                                                       "Atrato Rhabdo-like virus 3", 
                                                       "Joutseno rhabdovirus 1", 
                                                       "Retriended virus", 
                                                       "Primus virus", 
                                                       "XiangYun mono-chu-like virus 4")),]$tip <- TRUE
...
ggt2_hm <- ggt2_hm + geom_point(data = ggt2_hm$data[which(!is.na(ggt2_hm$data$novel_seq)),],
                          aes( x = (extracted_xlims[2] - 0.46), y = y), 
                          color = "black",
                          size=1, shape = 8)
...
ggt2_hm <- ggt2_hm + xlim((extracted_xlims1[1]), (extracted_xlims1[2]+1))
...
# 
# print(ggt2_hm)
ggt2_hm_file <- "/full_path_to/wd/RdRp_scan/analysis/phylo/Monjiviricetes_clade/Monjiviricetes_clade4_aa_aln.ggtree_hm.pdf"
ggsave(plot = ggt2_hm, filename = ggt2_hm_file, device = "pdf", width = 10, height = 14, units = "cm")

# ggt2_hm_file_legend <- "/full_path_to/wd/RdRp_scan/analysis/phylo/Monjiviricetes_clade/Monjiviricetes_clade4_aa_aln.ggtree_hm_legend.pdf"
...
print(ggt_sup2_hm)
ggt_sup2_hm_file <- "/full_path_to/wd/RdRp_scan/analysis/phylo/Monjiviricetes_clade/Monjiviricetes_clade4_aa_aln.ggtree_hm_supplementary.pdf"
