...
################## Tree and metadata 
tree_file <- "/full_path_to/wd/RdRp_scan/analysis/phylo/Nodamuvirales_clade/tree/aa_tree/Nodamuvirales_clade1_aa_aln_outg.fasta.gap50_clean.fasta.contree"
...
###### Metadata
tip_mdf_brief <- read_delim("/full_path_to/wd/RdRp_scan/analysis/phylo/Nodamuvirales_clade/updated_metadata_with_novel_seq1.txt", 
                  delim = "\t")
...
# gb_parse_gb_parsed_metadata_file <- "/full_path_to/wd/RdRp_scan/analysis/phylo/Nodamuvirales_clade/Nodamuvirales_clade1_to_parse_metadata.gb"
# count = 0
# entry_ends = FALSE
# Location <- NULL
# host_taxon <- NULL
...
# 
# write.table(file = "/full_path_to/wd/RdRp_scan/analysis/phylo/Nodamuvirales_clade/Nodamuvirales_clade1_gb_parsed_metadata.txt",
#             x = gb_parsed_metadata, sep = "\t",append = F, quote = F,row.names = F, col.names = T)

gb_parsed_metadata <- read_delim("/full_path_to/wd/RdRp_scan/analysis/phylo/Nodamuvirales_clade/Nodamuvirales_clade1_gb_parsed_metadata.txt",
                                 delim = "\t")
...
tip_mdf_brief$taxize_family <- NA
tip_mdf_brief$taxize_class <- NA
tip_mdf_brief$taxize_phylum <- NA
...
#' # from https://github.com/ropensci/rentrez/blob/master/R/api_keys.r
key="b846efefdc1e057fae015142125e6842d108"
...
annotate_family_class <- function(df){
  df =tip_mdf_brief_na
  tryCatch(
    expr = {
      for (ht in sort(unique(df$host_taxon))){
        some_function_that_may_fail <- function(ht) {
          temp_tax_df <- tax_name(query = ht, get = c("family", "class", "phylum"), db = "ncbi")
          return(temp_tax_df)
        }
        temp_tax_df <- NULL
        attempt <- 1
        while( is.null(temp_tax_df) && attempt <= 8 ) {
          attempt <- attempt + 1
          try(
            temp_tax_df <- some_function_that_may_fail(ht)
          )
        } 
        
        df[which(df$host_taxon == ht),]$taxize_family <- temp_tax_df$family
        df[which(df$host_taxon == ht),]$taxize_class <- temp_tax_df$class
        df[which(df$host_taxon == ht),]$taxize_phylum <- temp_tax_df$phylum
      }
    },
    error = function(e){ 
      return(df)
...
# tip_mdf_brief_na <- tip_mdf_brief[which(is.na(tip_mdf_brief$taxize_class) & 
#                                           is.na(tip_mdf_brief$taxize_family) &
#                                           is.na(tip_mdf_brief$taxize_phylum) &
#                                           !is.na(tip_mdf_brief$host_taxon) & 
#                                           !tip_mdf_brief$host_taxon == ""),]
# tip_mdf_brief_already_annotated <- tip_mdf_brief[which(!tip_mdf_brief$nt_seq_ID %in% tip_mdf_brief_na$nt_seq_ID),]
# tip_mdf_brief_in_progress1 <- annotate_family_class(tip_mdf_brief_na)
# tip_mdf_brief <- bind_rows(tip_mdf_brief_already_annotated, tip_mdf_brief_in_progress1)
# 
# write.table(x = tip_mdf_brief, file = "/full_path_to/wd/RdRp_scan/analysis/phylo/Nodamuvirales_clade/Nodamuvirales_clade1_taxize_annotated.txt", 
#             sep = "\t", append = F, quote = F, row.names = F, col.names = T)
tip_mdf_brief <- read_delim("/full_path_to/wd/RdRp_scan/analysis/phylo/Nodamuvirales_clade/Nodamuvirales_clade1_taxize_annotated.txt",
                            delim = "\t")
...
tip_mdf_brief[which(!tip_mdf_brief$taxize_family == "Culicidae" & !tip_mdf_brief$host_genus == "Culicidae"),]$host_taxon1 <- tip_mdf_brief[which(!tip_mdf_brief$taxize_family == "Culicidae" & !tip_mdf_brief$host_genus == "Culicidae"),]$taxize_family
tip_mdf_brief[which(!tip_mdf_brief$taxize_family %in% c("Culicidae", "Ixodidae") & 
                      !tip_mdf_brief$host_taxon1 %in% c("Culicidae", "Ixodidae") ),]$host_taxon1 <- tip_mdf_brief[which(!tip_mdf_brief$taxize_family %in% c("Culicidae", "Ixodidae") & 
                                                                                                                          !tip_mdf_brief$host_taxon1 %in% c("Culicidae", "Ixodidae") ),]$taxize_phylum
...
# tip_mdf_brief[which(tip_mdf_brief$host_taxon1 %in% c("Paris", "Vitis","Cladosporium","Erysiphe", "Plasmopara", "Rhizoctonia")), ]$host_taxon1 <- "Plantae/Fungi/Chromista"
# tip_mdf_brief[which(tip_mdf_brief$host_taxon1 %in% c("Dermacentor", "Ixodes", "Rhipicephalus")), ]$host_taxon1 <- "Ixodida"
...
# tip_mdf_brief[which(tip_mdf_brief$host_taxon1 %in% c("Arthropoda", "Insecta", "Acanthosoma", "Acyrthosiphon",
#                                                      "Cnaphalocrocis", "Diadegma", "Drosophila",
#                                                      "Frankliniella", "Gerridae", "Glycaspis",
#                                                      "Hyalopterus", "Odonata", "Peyerimhoffina",
#                                                      "Podagrion", "Scirpophaga", "Sesamia",
...
#                                                      "Araneae", "Ochthera", "Conocephalus", "Scutigera")), ]$host_taxon1 <- "Arthropoda"
...
# tip_mdf_brief[which(tip_mdf_brief$host_taxon1 %in% c("Larvivora", "bird feces", "Phylloscopus")), ]$host_taxon1 <- "bird anal swab/feces"
# tip_mdf_brief[which(tip_mdf_brief$host_taxon1 %in% c("sediment", "soil")), ]$host_taxon1 <- "sediment/soil"
...
tip_mdf_brief[which(tip_mdf_brief$Country %in% 
                      c("Sweden", "Finland", "Spain", "Serbia", "Greece", 
                        ...
                        "Austria",  "Slovakia", "Russia", "France", "Croatia", "Portugal", "Slovenia", "United Kingdom")),
]$Region <- "Europe"
...
tip_mdf_brief[which(tip_mdf_brief$Country %in% 
                      c("India", "China", "Malaysia", "Thailand", "Cambodia", "Viet Nam", "Taiwan",
                        #"Cambodia:Battambang & Pailin", "Cambodia:Kompong Thom", "Cambodia:Preah Vihear", "Cambodia:Pursat", "Cambodia:Sihanuokville",
                        "South Korea", "Japan","Indonesia", "Australia", "New Zealand",  "Papua New Guinea")),
]$Region <- "South, Southeast Asia, and Oceania"
...
#             file = "/full_path_to/wd/RdRp_scan/analysis/phylo/Nodamuvirales_clade/clades.tsv",
#             append = F, sep = "\t", col.names = T,
#             row.names = F, quote = F)
# 
# clades_df <- read_delim("/full_path_to/wd/RdRp_scan/analysis/phylo/Nodamuvirales_clade/clades.tsv", delim = '\t')
# 
...
ggt2_hm <- gheatmap(ggt2, genus_matrix, offset=0.2, width=1, font.size=2,
...
ggt2_hm <- ggt2_hm + geom_point(data = ggt2_hm$data[which(!is.na(ggt2_hm$data$novel_seq)),],
                          aes( x = (extracted_xlims[2] - 0.5), y = y), 
                          color = "black",
                          size=1, shape = 8)
...
ggt2_hm <- ggt2_hm + scale_fill_manual(breaks=c(# 'Anopheles',
                                                ...
                                                # 'Culiseta',
                                                # 'Culex pipiens', 'Culex pseudovishnui', 'Culex quinquefasciatus',
                                                'Culex',
                                                # 'Uranotaenia',
                                                'Psorophora',
...
# 
# print(ggt2_hm)
ggt2_hm_file <- "/full_path_to/wd/RdRp_scan/analysis/phylo/Nodamuvirales_clade/Nodamuvirales_clade1_aa_aln.ggtree_hm.pdf"
ggsave(plot = ggt2_hm, filename = ggt2_hm_file, device = "pdf", width = 20, height = 20, units = "cm")

# ggt2_hm_file_legend <- "/full_path_to/wd/RdRp_scan/analysis/phylo/Nodamuvirales_clade/Nodamuvirales_clade1_aa_aln.ggtree_hm_legend.pdf"
...
# 
# print(ggt2_hm)
ggt2_hm_file <- "/full_path_to/wd/RdRp_scan/analysis/phylo/Nodamuvirales_clade/Nodamuvirales_clade1_aa_aln.ggtree_hm1.pdf"
ggsave(plot = ggt2_hm, filename = ggt2_hm_file, device = "pdf", width = 10, height = 20, units = "cm")
...
print(ggt_sup2_hm)
ggt_sup2_hm_file <- "/full_path_to/wd/RdRp_scan/analysis/phylo/Nodamuvirales_clade/Nodamuvirales_clade1_aa_aln.ggtree_hm_supplementary.pdf"
