...
################## Tree and metadata 
tree_file <- "/full_path_to/wd/RdRp_scan/analysis/phylo/Phasi_clade/tree/aa_tree/Phasi_clade_aa_v3_aln.fasta.gap_clean.fasta.contree"
...
###### Metadata
tip_mdf_brief <- read_delim("/full_path_to/wd/RdRp_scan/analysis/phylo/Phasi_clade/updated_metadata.txt", 
                  delim = "\t")
...
virus_clade <- c("Phasi Charoen-like phasivirus", "Ackless virus",
                 "Guadeloupe phasivirus", "Adableaps virus", 
                 "Kimberley phasivirus", "Broome phasivirus 1", "Badu phasivirus",
                 "Wutai mosquito phasivirus", 
                 "Fly phasivirus", "Dipteran phasivirus")

clades_df <- tibble(virus_clade)
clades_df$node <- NA

clades_df[which(clades_df$virus_clade == "Phasi Charoen-like phasivirus"), ]$node <- 86
clades_df[which(clades_df$virus_clade == "Ackless virus"), ]$node <- 70
clades_df[which(clades_df$virus_clade == "Guadeloupe phasivirus"), ]$node <- 71

clades_df[which(clades_df$virus_clade == "Adableaps virus"), ]$node <- 154

clades_df[which(clades_df$virus_clade == "Kimberley phasivirus"), ]$node <- 76
clades_df[which(clades_df$virus_clade == "Broome phasivirus 1"), ]$node <- 75
clades_df[which(clades_df$virus_clade == "Badu phasivirus"), ]$node <- 74

clades_df[which(clades_df$virus_clade == "Wutai mosquito phasivirus"), ]$node <- 158

clades_df[which(clades_df$virus_clade == "Fly phasivirus"), ]$node <-80
clades_df[which(clades_df$virus_clade == "Dipteran phasivirus"), ]$node <- 79


write.table(x = clades_df,
            file = "/full_path_to/wd/RdRp_scan/analysis/phylo/Phasi_clade/clades.tsv",
            append = F, sep = "\t", col.names = T,
            row.names = F, quote = F)

clades_df <- read_delim("/full_path_to/wd/RdRp_scan/analysis/phylo/Phasi_clade/clades.tsv", delim = '\t')
...
# 
# print(ggt2_hm)
ggt2_hm_file <- "/full_path_to/wd/RdRp_scan/analysis/phylo/Phasi_clade/Phasi_clade_aa_v3_aln.ggtree_hm.pdf"
ggsave(plot = ggt2_hm, filename = ggt2_hm_file, device = "pdf", width = 18, height = 20, units = "cm")

# ggt2_hm_file_legend <- "/full_path_to/wd/RdRp_scan/analysis/phylo/Phasi_clade/Phasi_clade_aa_v3_aln.ggtree_hm_legend.pdf"
...
print(ggt_sup2_hm)
ggt_sup2_hm_file <- "/full_path_to/wd/RdRp_scan/analysis/phylo/Phasi_clade/Phasi_clade_aa_v3_aln.ggtree_hm_supplementary.pdf"
