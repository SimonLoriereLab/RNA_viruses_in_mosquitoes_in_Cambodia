...
################## Tree and metadata 
tree_file <- "/full_path_to/wd/RdRp_scan/analysis/phylo/Sobeli_clade/tree/aa_tree/Sobeli_clade_aa_v211_aln_outg.fasta.gap50_clean.fasta.contree"
...
tip_mdf_brief[which(tip_mdf_brief$nt_accession == "MT568531"),]$host_taxon <- "Culex bitaeniorhynchus"
tip_mdf_brief[which(tip_mdf_brief$nt_accession == "MT568532"),]$host_taxon <- "Culex bitaeniorhynchus"
tip_mdf_brief[which(tip_mdf_brief$nt_accession == "MW199223"),]$host_taxon <- "Arthropoda"
tip_mdf_brief[which(tip_mdf_brief$nt_accession == "MW520388"),]$host_taxon <- "Culex impudicus"
tip_mdf_brief[which(tip_mdf_brief$nt_accession == "MN661091"),]$host_taxon <- "Coquillettidia venezuelensis"
tip_mdf_brief[which(tip_mdf_brief$nt_accession == "MN661093"),]$host_taxon <- "Coquillettidia"
tip_mdf_brief[which(tip_mdf_brief$nt_accession == "MN661095"),]$host_taxon <- "Psorophora"
tip_mdf_brief[which(tip_mdf_brief$nt_accession == "MK440657"),]$host_taxon <- "Culex"
...
virus_clade <- c("MW199223", "Humaita-Tubiacanga virus",
                 "Atrato Sobemo-like virus 3", "Atrato Sobemo-like virus 4",
                 "Pyongtaek Culex Solemovirus", 
                 "Broome luteo-like virus 1", "Pine Lake virus",
                 "Culex mosquito virus 6", "Berrek virus", 
                 "Culex impudicus luteo-like virus", 
                 "Pyongtaek Culex luteo-like virus",
                 "Culex luteo-like virus", "Hubei sobemo-like virus 41")

clades_df <- tibble(virus_clade)
clades_df$node <- NA

clades_df[which(clades_df$virus_clade == "MW199223"), ]$node <- 61
clades_df[which(clades_df$virus_clade == "Humaita-Tubiacanga virus"), ]$node <- 101
clades_df[which(clades_df$virus_clade == "Atrato Sobemo-like virus 3"), ]$node <- 100
clades_df[which(clades_df$virus_clade == "Atrato Sobemo-like virus 4"), ]$node <- 37
clades_df[which(clades_df$virus_clade == "Pyongtaek Culex Solemovirus"), ]$node <- 36
clades_df[which(clades_df$virus_clade == "Broome luteo-like virus 1"), ]$node <- 99
clades_df[which(clades_df$virus_clade == "Pine Lake virus"), ]$node <- 32
clades_df[which(clades_df$virus_clade == "Culex mosquito virus 6"), ]$node <- 94
clades_df[which(clades_df$virus_clade == "Berrek virus"), ]$node <- 26
clades_df[which(clades_df$virus_clade == "Culex impudicus luteo-like virus"), ]$node <- 25
clades_df[which(clades_df$virus_clade == "Pyongtaek Culex luteo-like virus"), ]$node <- 24
clades_df[which(clades_df$virus_clade == "Culex luteo-like virus"), ]$node <- 87
clades_df[which(clades_df$virus_clade == "Hubei sobemo-like virus 41"), ]$node <- 66
...
clades_df$tip <- FALSE
clades_df[which(clades_df$virus_clade_essential %in% c("MW199223",
                                                       "Atrato Sobemo-like virus 4",
                                                       "Pyongtaek Culex Solemovirus",
                                                       "Pine Lake virus",
                                                       "Berrek virus", 
                                                       "Culex impudicus luteo-like virus",
                                                       "Pyongtaek Culex luteo-like virus")),]$tip <- TRUE
...
ggt2_hm <- gheatmap(ggt2, genus_matrix, offset=0.225, width=0.22, font.size=2,
...
# 
# print(ggt2_hm)
ggt2_hm_file <- "/full_path_to/wd/RdRp_scan/analysis/phylo/Sobeli_clade/Sobeli_clade_aa_v211_aln.ggtree_hm.pdf"
ggsave(plot = ggt2_hm, filename = ggt2_hm_file, device = "pdf", width = 10, height = 14, units = "cm")

# ggt2_hm_file_legend <- "/full_path_to/wd/RdRp_scan/analysis/phylo/Sobeli_clade/Sobeli_clade_aa_v211_aln.ggtree_hm_legend.pdf"
...
print(ggt_sup2_hm)
ggt_sup2_hm_file <- "/full_path_to/wd/RdRp_scan/analysis/phylo/Sobeli_clade/Sobeli_clade_aa_v211_aln.ggtree_hm_supplementary.pdf"
