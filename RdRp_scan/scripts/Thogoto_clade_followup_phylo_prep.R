...
Thogoto_clade_df <- read_delim("/full_path_to/wd/RdRp_scan/analysis/phylo/Thogoto_clade/Thogoto_clade_novel_sequence_data.txt")
...
read_map_df_Thogoto_clade <- read_map_df[which(read_map_df$ref_id %in% Thogoto_clade_df$contig_id & read_map_df$selected == "yes"),]



read_map_df_Thogoto_clade$RdRp_scan_map_consensus_id <- paste0(read_map_df_Thogoto_clade$ref_id, "_",
                      read_map_df_Thogoto_clade$sample_id, "_RdRpScan_Round1")

read_map_df_Thogoto_clade$contig_id <- read_map_df_Thogoto_clade$ref_id

consensus_df <- left_join(read_map_df_Thogoto_clade, Thogoto_clade_df, by  = "contig_id")
...
extracted_consensus_directory <- "/full_path_to/wd/RdRp_scan/analysis/phylo/Thogoto_clade/extracted_cons1_renamed/"
dir.create(extracted_consensus_directory)
extracted_orf_nt_directory <- "/full_path_to/wd/RdRp_scan/analysis/phylo/Thogoto_clade/extracted_cons1_orf_nt/"
dir.create(extracted_orf_nt_directory)
extracted_orf_aa_directory <- "/full_path_to/wd/RdRp_scan/analysis/phylo/Thogoto_clade/extracted_cons1_orf_aa/"
...
write.table(x = consensus_df, file = "/full_path_to/wd/RdRp_scan/analysis/phylo/Thogoto_clade/consensus_df.txt", 
...
rdrp_scan_clade_accns <- str_remove(str_remove(str_remove(str_split(consensus_df$ref_suggestions, ",")[[1]], "[[:print:]]+Insthoviricetes_"), "[[:print:]]+NOT_ASSIGNED_"), "\\.[[:digit:]]{1}\\_[[:print:]]+$")

refs <- sort(unique(c(blastp_df2$sseqid, rdrp_scan_clade_accns)))

##### add sequences from publications on Orthomyxoviridae
################## Tree from Dudas & Batson 2022
tree_file <- "/full_path_to/wd/RdRp_scan/analysis/phylo/Thogoto_clade/Dudas_2022_PB1_increasing_midpoint.nwk"
...
for (tl in tip_labels){
  if (start == F){
    if (tl == "'AHB34061.1|Aransas_Bay_virus'"){
      start <- T
      tl_select <- c(tl_select, tl)
    }
  }else{
    if(tl == "'QQN90103.1|Soybean_thrips_thogotovirus_1'"){
      start <- F
      tl_select <- c(tl_select, tl)
    }else{
      tl_select <- c(tl_select, tl)
...
#### Reimport metadata
NCBI_Virus_mdf <- read_csv("/full_path_to/wd/RdRp_scan/analysis/phylo/Thogoto_clade/Thogoto_metadata_public.csv")
...
selected_mdf_upd_novel[which(selected_mdf_upd_novel$nt_seq_ID == "NC_006495_Thogotovirus_thogotoense"),]$Country <- "Italy"
selected_mdf_upd_novel[which(selected_mdf_upd_novel$nt_seq_ID == "NC_006495_Thogotovirus_thogotoense"),]$Year <- "1969"
selected_mdf_upd_novel[which(selected_mdf_upd_novel$nt_seq_ID == "NC_006495_Thogotovirus_thogotoense"),]$host_taxon <- "Rhiphicephalus"
selected_mdf_upd_novel[which(selected_mdf_upd_novel$nt_seq_ID == "NC_006495_Thogotovirus_thogotoense"),]$host_genus <- "Rhiphicephalus"

selected_mdf_upd_novel[which(selected_mdf_upd_novel$nt_seq_ID == "KC506163_Aransas_Bay_virus"),]$Country <- "USA"
selected_mdf_upd_novel[which(selected_mdf_upd_novel$nt_seq_ID == "KC506163_Aransas_Bay_virus"),]$Year <- "1975"
selected_mdf_upd_novel[which(selected_mdf_upd_novel$nt_seq_ID == "KC506163_Aransas_Bay_virus"),]$host_taxon <- "Ornithodoros capensis"
selected_mdf_upd_novel[which(selected_mdf_upd_novel$nt_seq_ID == "KC506163_Aransas_Bay_virus"),]$host_genus <- "Ornithodoros"

selected_mdf_upd_novel[which(selected_mdf_upd_novel$nt_seq_ID == "MT628417_Thogotovirus_dhoriense"),]$Country <- "Kyrgyzstan"
selected_mdf_upd_novel[which(selected_mdf_upd_novel$nt_seq_ID == "MT628417_Thogotovirus_dhoriense"),]$Year <- "1974"
selected_mdf_upd_novel[which(selected_mdf_upd_novel$nt_seq_ID == "MT628417_Thogotovirus_dhoriense"),]$host_taxon <- "Hyalomma plumbeum"
selected_mdf_upd_novel[which(selected_mdf_upd_novel$nt_seq_ID == "MT628417_Thogotovirus_dhoriense"),]$host_genus <- "Hyalomma"

selected_mdf_upd_novel[which(selected_mdf_upd_novel$nt_seq_ID == "HM627170_Thogotovirus_josense"),]$Country <- "Nigeria"
selected_mdf_upd_novel[which(selected_mdf_upd_novel$nt_seq_ID == "HM627170_Thogotovirus_josense"),]$Year <- "1967"
selected_mdf_upd_novel[which(selected_mdf_upd_novel$nt_seq_ID == "HM627170_Thogotovirus_josense"),]$host_taxon <- "Bos/Amblyomma/Rhipicephalus"
selected_mdf_upd_novel[which(selected_mdf_upd_novel$nt_seq_ID == "HM627170_Thogotovirus_josense"),]$host_genus <- "Bos/Amblyomma/Rhipicephalus"



write.table(x = selected_mdf_upd_novel, file = "/full_path_to/wd/RdRp_scan/analysis/phylo/Thogoto_clade/updated_metadata_with_novel_seq.txt", 
            append = F, quote = F, sep = "\t", row.names = F, col.names = T)
...
#### Put the file here!
cds_fasta_file <- "/full_path_to/wd/RdRp_scan/analysis/phylo/Thogoto_clade/Thogoto_clade_refs_cds_nt.txt"


### There might be more automatic solutions for CDS extractions with EMBOSS or CL entrez. But web NCBI is the fastest for now.

############################################################################################
############################################################################################
############################################################################################
cds_clade_dir <- "/full_path_to/wd/RdRp_scan/analysis/phylo/Thogoto_clade/ref_cds_nt"
dir.create(cds_clade_dir)
aa_clade_dir <- "/full_path_to/wd/RdRp_scan/analysis/phylo/Thogoto_clade/ref_cds_aa"
