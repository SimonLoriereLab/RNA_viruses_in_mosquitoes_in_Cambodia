...
##### v1.2.3 newer lineage.csv does not have 'species1', column numbers corrected
## "/full_path_to/db/ncbi_lineages_2021-07-23.csv"
...
write.table(x = df_joined_lineage, file = gzfile(file.path(args[3], paste0("lineage_complete.", s_end, ".gz"))),  #also altered to account for fullpath input
...
write.table(x = df_joined_lineage_collapsed, file = gzfile(file.path(args[3],paste0("lineage_collapsed.",s_end,".gz"))), #same alterations as above 
            quote = FALSE, sep = "\t", row.names = FALSE, col.names = TRUE, append = FALSE)
